...
export const SellerStudioView: React.FC<SellerStudioViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-slate-900 rounded-3xl p-6 sm:p-8 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border border-slate-800 shadow-sm">
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <h1 className="text-xl sm:text-2xl font-bold">Pattadar Seller Studio</h1>
        ...
        <div className="space-y-4">
          {sellerProperties.length === 0 ? (
            ...
            sellerProperties.map((property) => (
              <div
                key={property.id}
                className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-indigo-200 transition-all"
              >
                <div className="flex items-start space-x-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center space-x-2">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-900 text-white capitalize">
                        {property.purpose}
                      </span>
                    ...
                    <p className="text-xs font-bold text-slate-900">
                      Asking: {formatINR(property.askingPrice)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 self-end sm:self-center">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-semibold flex items-center space-x-1 shadow-2xs cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-rose-600 hover:bg-rose-50 border border-rose-200 shadow-2xs transition-colors cursor-pointer"
              ...
              {conversations.map((conv) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-2xl border p-5 shadow-xs hover:border-indigo-500 hover:shadow-md transition-all cursor-pointer flex flex-col justify-between group ${
                      hasPhoneRequest ? 'border-amber-300 ring-1 ring-amber-200 bg-amber-50/20' : 'border-slate-200/80'
                    }`}
                  >
                    <div className="space-y-2.5">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          {hasPhoneRequest && (
                            <span className="px-2 py-0.5 rounded-md bg-amber-100 text-amber-800 text-[10px] font-bold flex items-center space-x-1 border border-amber-200">
        ...
        <div className="space-y-4">
          {siteVisits.length === 0 ? (
            ...
            siteVisits.map((visit) => (
              <div
                key={visit.id}
                className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                <div className="space-y-1.5">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs font-bold text-slate-900">{visit.propertyTitle}</span>
                  </div>
                  ...
                  {visit.status === 'REQUESTED' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={updatingVisitId === visit.id}
                        className="px-3 py-1.5 rounded-xl border border-slate-200 text-slate-700 text-xs font-medium hover:bg-slate-50 cursor-pointer"
                      >
                        Decline
                      </button>
                    </>
      ...
      {statusNotification && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 text-white px-5 py-3.5 rounded-2xl shadow-xl border border-slate-800 flex items-center space-x-3 text-xs sm:text-sm font-medium animate-in fade-in slide-in-from-bottom-3 duration-200">
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 space-y-6 relative animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center space-x-3.5">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">Confirm Land Deletion</h3>
